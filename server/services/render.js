...
axios.get('http://localhost:3000/api/users') //this get req return promise
.then(function(response){
console.log(response.data);
res.render('index', { users : response.data });  //pass the data with this render method
...
exports.update_user = (req, res) =>{
    axios.get('http://localhost:3000/api/users', { params : { id : req.query.id }})//get this data (we pass the id var here)
        .then(function(userdata){
            res.render("update_user", { user : userdata.data}) // then pass that data to this user object
        })
        .catch(err =>{
            res.send(err);
        })
